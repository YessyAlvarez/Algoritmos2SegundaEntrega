...
public class Lista {
    ...
    public void agregarInicio(int vd, int peso){
    ...
    public void agregarFinal(NodoLista nodo){
        ...
        else
        {
            ...
            while (aux.getSig() != null)
                aux=aux.getSig();
            NodoLista nuevo = new NodoLista(nodo.getDato(), nodo.getCantidad());
            aux.setSig(nuevo);
            this.fin=nuevo;
        }
    }
...
/*****Otros Métodos (iterativos)*****/
    ...
    public void agregarOrd(String n){
        ...
        while (aux.getSig()!=null && aux.getSig().getDato().equals(n))
            aux=aux.getSig();
        NodoLista nuevo=new NodoLista("",1);
	...
	/*	 //lista vacia o primer elemento es mayor o igual => agrego al ppio
	 ...
	 public void agregarOrd(NodoLista n){
	        
	        //lista vacìa o primer elemento es mayor o igual => agrego al ppio
	        if (this.esVacia() || this.inicio.getCantidad() <= n.getCantidad()){
	            this.agregarInicio(n);
	            return;
	        }
	        if (this.inicio.getCantidad() > n.getCantidad()){   //ùltimo elemento es menor o igual => agrego al final
	            this.agregarFinal(n);
	            return;
	        }
	        NodoLista aux=this.inicio;
	        while (aux.getSig()!=null && aux.getSig().getCantidad() < n.getCantidad()){
	        	NodoLista nuevo = new NodoLista(n.getDato(), n.getCantidad());
	            nuevo.setSig(aux.getSig());
	            aux.setSig(nuevo);
	        }
	    }
